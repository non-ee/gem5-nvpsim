/**
 * Simple Wildlife Audio Classifier
 *
 * Detects what type of animal is present based on sound frequency patterns
 */
...
// ==================== CONFIGURATION ====================
#define SAMPLE_COUNT         100    // Number of audio samples to analyze
#define SAMPLE_INTERVAL_MS   50     // 50ms between samples = 20Hz
...
int main(void) {
    ...
    // 2. Analyze frequency
    printf("Analyzing frequency...\n");
    uint32_t dominant_freq = analyze_frequency();

    // 3. Classify animal
    printf("Classifying animal...\n");
    detected_animal = classify_animal(dominant_freq);

    // 4. Display and transmit result
    printf("Dominant frequency: %lu Hz\n", dominant_freq);
